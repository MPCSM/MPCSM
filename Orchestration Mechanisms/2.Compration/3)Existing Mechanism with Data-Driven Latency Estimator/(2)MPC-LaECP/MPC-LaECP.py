...
for thisTotalCount in datasums:
    ...
    oneGraphApp.setMicroServices([
        ["fault-source", True, False, 0, faultDetectionCPU[0], faultDetectionMem[0], 1, 7464602, 0, 0, 0.05, 1,
         thisTotalCount],  # 0
        ...
        ["fault-losso", False, False, 1, faultDetectionCPU[1], faultDetectionMem[1], 3, 515318 * 3, 1, 7464602, 51, 1,
         thisTotalCount],  # 1
        ["wafer-auto", False, False, 1, waferInspectionCPU[1], waferInspectionCPU[1], 1, 7132, 1, 237880, 19.2, 1,
         thisTotalCount],  # 7
        ["wafer-static", False, False, 1, waferInspectionCPU[2], waferInspectionCPU[2], 1, 36632, 1, 314889, 141, 1,
         thisTotalCount],  # 8
        # ["rul-preprocess", False, False, 1, rulPredictionCPU[1], rulPredictionMem[1], 1, 63565202, 1, 3509033, 0.02, 5, 29],
        ["fault-knn", False, False, 2, faultDetectionCPU[2], faultDetectionMem[2], 1, 12752, 1, 515318, 19.1, 1,
         thisTotalCount],  # 2
        ["fault-svm", False, False, 2, faultDetectionCPU[3], faultDetectionMem[3], 1, 12752, 1, 515318, 1, 1,
         thisTotalCount],  # 3
        ["fault-xgboost", False, False, 2, faultDetectionCPU[4], faultDetectionMem[4], 1, 12752, 1, 515318, 0.5, 1,
         thisTotalCount],  # 4
        ["wafer-cnn", False, False, 2, waferInspectionCPU[3], waferInspectionCPU[3], 1, 263, 1, 7132, 34, 1,
         thisTotalCount],  # 9
        ["wafer-svm", False, False, 2, waferInspectionCPU[4], waferInspectionCPU[4], 1, 611, 1, 36632, 7, 1,
         thisTotalCount],  # 10
        ["fault-ensemble", False, True, 3, faultDetectionCPU[5], faultDetectionMem[5], 0, 0, 3, 12752 * 3, 49, 1,
         thisTotalCount],  # 5
    ...
    def oneLatency(uplatency,bandwidth, microSubvolum, serviceName, device, deviceCpuPercent,microComponent):
        comm = microSubvolum / (bandwidth[0] * 1024 * 1024 / 8)
        if(device==5):
        ...
        total = comm + compute+uplatency
        return total;


    def twoLatency(upLatencys,bandwidth, microSubvolum, serviceName, device, deviceCpuPercent,microComponent):
        ...
        if (device == 5):
            ...
            X_test_quadratic = quadratic_featurizer.fit_transform([[deviceCpuPercent]])
            compute = xrf.predict(X_test_quadratic)
        total1 = comm1 + compute+upLatencys[0]

        comm2 = microSubvolum / (bandwidth[1] * 1024 * 1024 / 8)

        total2 = comm2 + compute+upLatencys[1]
    ...
    def threeLatency(upLatencys,bandwidth, microSubvolum, serviceName, device, deviceCpuPercent,microComponent):
        ...
        if (device == 5):
            ...
            X_test_quadratic = quadratic_featurizer.fit_transform([[deviceCpuPercent]])
            compute = xrf.predict(X_test_quadratic)
        latency1 = comm1 + compute+upLatencys[0]
        comm2 = microSubvolum / (bandwidth[1] * 1024 * 1024 / 8)

        latency2 = comm2 + compute+upLatencys[1]

        comm3 = microSubvolum / (bandwidth[2] * 1024 * 1024 / 8)

        latency3 = comm3 + compute+upLatencys[2]
    ...
    for micro in oneGraphAppServices:
        ...
        if (micro.getServiceName() == "fault-xgboost"):
            ...
            tempTotal = oneLatency(upLatancys[0],
            ...
            for device in devices[0:3]:
                ...
                if (micro.getCPUDemand() <= device.getCPURemain()):
                    if (innodeSize == 1):
                        ...
                        tempTotal = twoLatency(upLatancys,
                            bd, microSubVolum, microServiceName, subDeviceId, subDeviceCpuPercent,microComponent)
                        ...
                        tempTotal = threeLatency(upLatancys,
                            bd, microSubVolum, microServiceName, subDeviceId, subDeviceCpuPercent, microComponent)
    ...
    sourcelossofilename = 'result/allplacement/' + str(thisTotalCount) + '-' + thisType + '-MPC-LaECP' + str(
        s) + "-allPlacement.csv"
    features1 = ["placement"]
    ...
    sourcelossofilename = 'result/infoForeachPlace/MPC-LaECP-placement-'+str(thisTotalCount)+'.csv'

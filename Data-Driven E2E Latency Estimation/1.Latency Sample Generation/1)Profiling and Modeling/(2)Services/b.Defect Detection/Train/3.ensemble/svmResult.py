...
if __name__ == "__main__":
    df_withpattern = pd.read_pickle("./wafermap_withlabel.pkl")[:10]
    ...
    #print("test",df_withpattern_copy[0])
    ...
    print(df_all[:1])
    print(type(df_all.fea_cub_mean))
    #df_all.fea_cub_mean.as_matrix()
    ...
    fea_all = np.concatenate(( np.array(b), np.array(c), np.array(d)), axis=1)  # 59 in total
    print("fea:",fea_all[:2])
    label = [df_all.failureNum[i] for i in range(df_all.shape[0])]
    label = np.array(label)

    #df_all.to_csv('Result1.csv',columns=['fea_cub_mean','fea_cub_std','fea_geom','failureType'])
    ...
    # y = clf2.predict(X)
    # attributes=["svmResult"]
    # pd.DataFrame()
    # wafersvmResult=pd.DataFrame(columns=attributes,data=y)
    # wafersvmResult.to_csv("wafersvmAttribute.csv")

...
placementMixEdge=pd.read_csv("dbname-bytotalcount/"+str(totalCount)+"-db_name-edge.csv").values
#Traverse the 69 decisions of the wafer
for i in range(0,69):
    ...
    for j in range(len(placementMixEdge)):
        if (placementMixEdge[j][1]==i):
            faultDecisionsEdge.append(placementMixEdge[j][2])
    if(len(faultDecisionsEdge)):
        continue;
    myclient = pymongo.MongoClient("mongodb://192.168.0.135:27017/")
    faultEdgedb = myclient[fault_db_name]
    datemill=1
    ...
    for faultDecision in faultDecisionsEdge:
        ...
        # --------------------------Count fault's send and receive topics and delay information--------------------------------------
        mycol = faultEdgedb["decision"+str(faultDecision)]
        ...
        statissvm=[]
        cnnstatisensemble=[]
        # --------------------------1.Count the fault's sending and receiving topics--------------------------------------
        ...
        for x in mydoc:
            ...
            lat1=lat1+knnTime[2]-knnTime[0]
            lat2=lat2+knnTime[4]-knnTime[0]
            lat3=lat3+svmTime[4]-svmTime[0]
            lat4=lat4+exgboostTime[4]-exgboostTime[0]
            lat5=lat5+max(ensembleTime[1]-knnTime[0],ensembleTime[1]-svmTime[0],ensembleTime[1]-exgboostTime[0])
        #losso
        label.append(lat1/totalCount)
        #knn
        label.append(lat2/totalCount)
        #svm
        label.append(lat3/totalCount)
        #xgboost
        label.append(lat4/totalCount)
        #ensemble
        triForkLabel.append(lat5/totalCount)

        data.append([totalCount, micocomponent[0], faultDateRate[0][3], faultDateRate[0][2], faultDateRate[0][1], faultDateRate[0][0], datemill,
                pi[0], pi[1], pi[2], pi[3], pi[4], picpurate, 0,
                topicVolume[ip0][0], topicVolume[ip0][1], topicVolume[ip0][2], topicVolume[ip0][3],
                topicVolume[ip0][4],
                cpurate[ip0],
                round(cpuNow[ip0] / cpuTotal[ip0],2),0,
                 totalCount,0.05,pi[3],pi[2],pi[1],pi[0],datemill])
        #knn
        data.append([totalCount, micocomponent[1], faultDateRate[1][3], faultDateRate[1][2], faultDateRate[1][1], faultDateRate[1][0], datemill,

                    topicVolume[ip0][0], topicVolume[ip0][1], topicVolume[ip0][2], topicVolume[ip0][3],
                    topicVolume[ip0][4], cpurate[ip0], round(cpuNow[ip0] / cpuTotal[ip0], 2),
                    topicVolume[ip1][0], topicVolume[ip1][1], topicVolume[ip1][2], topicVolume[ip1][3],
                    topicVolume[ip1][4], cpurate[ip1],
                    round(cpuNow[ip1] / cpuTotal[ip1], 2),lat1/totalCount,
                    totalCount, micocomponent[0], faultDateRate[0][3], faultDateRate[0][2], faultDateRate[0][1], faultDateRate[0][0], datemill])
        #svm
        data.append([totalCount, micocomponent[2], faultDateRate[2][3], faultDateRate[2][2], faultDateRate[2][1], faultDateRate[2][0], datemill,
                    topicVolume[ip0][0], topicVolume[ip0][1], topicVolume[ip0][2], topicVolume[ip0][3],
                    topicVolume[ip0][4], cpurate[ip0], round(cpuNow[ip0] / cpuTotal[ip0], 2),
                    topicVolume[ip2][0], topicVolume[ip2][1], topicVolume[ip2][2], topicVolume[ip2][3],
                    topicVolume[ip2][4], cpurate[ip2],
                    round(cpuNow[ip2] / cpuTotal[ip2], 2),lat1/totalCount,
                    totalCount, micocomponent[0], faultDateRate[0][3], faultDateRate[0][2], faultDateRate[0][1], faultDateRate[0][0], datemill])
        #xgboost
        data.append([totalCount, micocomponent[3], xgboostA[3], xgboostA[2], xgboostA[1], xgboostA[0], datemill,
                    topicVolume[ip0][0], topicVolume[ip0][1], topicVolume[ip0][2], topicVolume[ip0][3],
                    topicVolume[ip0][4], cpurate[ip0], round(cpuNow[ip0] / cpuTotal[ip0], 2),
                    topicVolume[2][0], topicVolume[2][1], topicVolume[2][2], topicVolume[2][3],
                    topicVolume[2][4], cpurate[2],
                    round(cpuNow[2] / cpuTotal[2], 2),lat1/totalCount,
                    totalCount, micocomponent[0], faultDateRate[0][3], faultDateRate[0][2], faultDateRate[0][1], faultDateRate[0][0], datemill])
        #ensemble
        triForkData.append(
            [totalCount, micocomponent[4], faultDateRate[3][3], faultDateRate[3][2], faultDateRate[3][1], faultDateRate[3][0], datemill,

            topicVolume[ip1][0], topicVolume[ip2][0], topicVolume[2][0],
            topicVolume[ip1][1], topicVolume[ip2][1], topicVolume[2][1],
            topicVolume[ip1][2], topicVolume[ip2][2], topicVolume[2][2],
            topicVolume[ip1][3], topicVolume[ip2][3], topicVolume[2][3],
            topicVolume[ip1][4], topicVolume[ip2][4], topicVolume[2][4],
            cpurate[ip1], cpurate[ip2], cpurate[2],
            round(cpuNow[ip1] / cpuTotal[ip1], 2), round(cpuNow[ip2] / cpuTotal[ip2], 2),
            round(cpuNow[2] / cpuTotal[2], 2),

            topicVolume[ip3][0], topicVolume[ip3][1], topicVolume[ip3][2], topicVolume[ip3][3],
            topicVolume[ip3][4], cpurate[ip3],

            round(cpuNow[ip3] / cpuTotal[ip3], 2),lat2/totalCount,lat3/totalCount,lat4/totalCount,
            totalCount, micocomponent[1], faultDateRate[1][3], faultDateRate[1][2], faultDateRate[1][1], faultDateRate[1][0], datemill,
            totalCount, micocomponent[2], faultDateRate[2][3], faultDateRate[2][2], faultDateRate[2][1], faultDateRate[2][0], datemill,
            totalCount, micocomponent[3], xgboostA[3], xgboostA[2], xgboostA[1], xgboostA[0], datemill])
        # --------------------------2.Account for the wafer's delay--------------------------------------
    ...
    dataPD.to_csv("features-bytotalcount/"+str(totalCount)+"-edge-fault-one-data.csv")
    labelPD=pd.DataFrame(columns=labelAtt,data=label)
    labelPD.to_csv("features-bytotalcount/"+str(totalCount)+"-edge-fault-one-label.csv")
    #fault three
    dataPD=pd.DataFrame(columns=dataAttTri,data=triForkData)
    dataPD.to_csv("features-bytotalcount/"+str(totalCount)+"-edge-three-data.csv")
    labelPD=pd.DataFrame(columns=labelAtt,data=triForkLabel)
    labelPD.to_csv("features-bytotalcount/"+str(totalCount)+"-edge-three-label.csv")

    # wafer one
    dataPD = pd.DataFrame(columns=dataAtt, data=waferdata)
    dataPD.to_csv("features-bytotalcount/"+str(totalCount) + "-edge-wafer-one-data.csv")
    labelPD = pd.DataFrame(columns=labelAtt, data=waferlabel)
    labelPD.to_csv("features-bytotalcount/"+str(totalCount) + "-edge-wafer-one-label.csv")

    # wafer two
    dataPD = pd.DataFrame(columns=dataAttTwo, data=twoForkdata)
    dataPD.to_csv("features-bytotalcount/"+str(totalCount) + "-edge-two-data.csv")
    labelPD = pd.DataFrame(columns=labelAtt, data=triForkLabel)
    labelPD.to_csv("features-bytotalcount/"+str(totalCount) + "-edge-two-label.csv")
...
for i in range(0,69):
    ...
    #The number of sent topics, the number of sent data, the number of received topics,
    #  the number of received data 0, 1, 2 and 3 respectively represent the four containers of 'losso',' KNN ',' SVM 'and 'ensemble'
    faultDateRate={
        0: [3,1545954,1,7464602],
        1: [1,12752,1,515318],
        2: [1,12752,1,515318],
    ...
    #fault one
    dataPD=pd.DataFrame(columns=dataAtt,data=data)
    dataPD.to_csv("features-bytotalcount/"+str(totalCount)+"-cloud-fault-one-data.csv")
    labelPD=pd.DataFrame(columns=labelAtt,data=label)
    labelPD.to_csv("features-bytotalcount/"+str(totalCount)+"-cloud-fault-one-label.csv")
    #fault three
    dataPD=pd.DataFrame(columns=dataAttTri,data=triForkData)
    dataPD.to_csv("features-bytotalcount/"+str(totalCount)+"-cloud-three-data.csv")
    labelPD=pd.DataFrame(columns=labelAtt,data=triForkLabel)
    labelPD.to_csv("features-bytotalcount/"+str(totalCount)+"-cloud-three-label.csv")
